...
export const ProgressBar: React.FC<ProgressBarProps> = ({
  ...
  return (
    <div className={cn("w-full", className)}>
      {showLabel && (
        <div className="flex justify-between text-xs font-medium text-gray-500 dark:text-gray-400 dark:text-gray-400 mb-1">
          <span>Progress</span>
          <span>{Math.round(percentage)}%</span>
        </div>
      )}
      <div className={cn("w-full bg-gray-100 dark:bg-gray-800 dark:bg-gray-800 rounded-full overflow-hidden", sizes[size])}>
        <div 
